...
#ifndef subgradient_hpp
#define subgradient_hpp
...
public:    
    ...
    int dsize;
    int iter_;
    unsigned int contgreen, contyellow, contred;
    ...
    int take_step(const double & teta);
    ...
    double sg_abs(const double &)const ;
    double sg_max(const double &,const double &)const;


    int sumVec(std::vector<double>& w,const std::vector<double> & v1, const std::vector<double> & v2, double scalar=1.0);
    double dotProduct(const std::vector<double> & v1, const std::vector<double> & v2);

    double readjust_target(double oldtarget);
    double compute_teta(const double & ddotg) const;
    void print_info();
