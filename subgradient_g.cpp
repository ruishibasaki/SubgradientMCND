...
void subgrad::print_info(){
    ...
    std::cout<<std::setprecision(15)<<iter_<<" L*= "<<lstar<<" target= "<<ub<<" L= "<<lcost<<" iter g,r = {"<<contgreen<<" , "<<" , "<<contred<<"}"<<std::endl;
}

double subgrad::sg_abs(const double & f){
    if (f<0.000) {
        return -f;
    }else return f;
}

double subgrad::sg_max(const double & a,const double & b){
    if (a<b) {
        return b;
    }else return a;
}
...
int subgrad::sumVec(std::vector<double>& w,const std::vector<double> & v1, const std::vector<double> & v2){
...
double subgrad::compute_stepsize(){
    ...
    stepsize = lambda*(ub - lstar)/viol;
    return stepsize;
}


int subgrad::take_step(){
    std::vector<double>::iterator itu = u.begin();
    std::vector<double>::const_iterator itd = g.begin();
    double stepsize=0;
...
int subgrad::solve(user_Provide & hooks){
    ...
    for (iter_ = 1; iter_ <= parm.maxsgriters; ++iter_) {  // main iteration
        ...
        viol =compute_norm(g);
        const bool primal_feas = (std::sqrt(viol) < parm.primal_abs_precision);
        ...
        retval = take_step(); // take a dual step
        if (retval < 0)  break;
        ...
        if (t_info.resident_size - maxMem > 0.00001) {
...
subgrad::subgrad(int primalsize, int dualsize, const char* filename, std::string instance){
    ...
    //const char *file = (instance.substr(instance.size()-13,1)+instance.substr(instance.size()-9,5)).c_str();
    //outfile.open("out");
}
